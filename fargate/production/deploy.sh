...
APPLICATION_PATH="$HOME/Documents/docker_on_aws_udemy/dockerzon"
...
REGISTRY="000986988155.dkr.ecr.eu-west-1.amazonaws.com"
...
function push_to_registry () {
  ...
  eval "$(aws --profile op ecr get-login  --no-include-email)"
...
function update_web_service () {
  ...
  aws  --profile op ecs update-service --cluster "${CLUSTER}" --service web \
...
function update_worker_service () {
  ...
  aws  --profile op ecs update-service --cluster "${CLUSTER}" --service worker \
    --task-definition worker --desired-count 1
}

function run_database_migration () {
  aws  --profile op ecs run-task --cluster "${CLUSTER}" --task-definition db-migrate --count 1
}
